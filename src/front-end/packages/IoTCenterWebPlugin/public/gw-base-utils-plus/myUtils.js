 ...
 */
import * as signalRAll from '@aspnet/signalr';
...
export default class myUtils {
    ...
    static streamLogin () {
        ...
        api.loginVideo({
            ...
            .catch(err => {
                this.$message({
                    type: 'warning',
                    title: this.$t('myUtils.msg.streamLogin[0]')
                });
    ...
    static async changeStyle (ele) {
        ...
        for (let item of iframe) {
            let cloneNode = stylee.cloneNode(true);
            item.contentWindow.document.getElementsByTagName("head")[0].removeChild(item.contentWindow.document.querySelector("#themeStyle"));
            item.contentWindow.document.getElementsByTagName("head")[0].appendChild(cloneNode);
        }
    }
     ...
     * @return {string}
     */
    static filterSpecialChar (value, str = '') {
        // 如果含有 [] ，先处理成\\[\\]
        if (str != '') {
