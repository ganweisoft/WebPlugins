...
export function getChartDayObjList () {
    let dataList = [];
    for (var i = 1; i < getCurrentMonthDays() + 1; i++) {
        // for (var i = 1; i < 4+1; i++) {
...
export function arrayRemoveRepeat (arry) {
    ...
    if (arry.length > 0) {
        ...
        arry.forEach((item, index) => {
            if (!obj.some((itemChild, indexChild) => { return itemChild.equipno == item.equipno; })) {
                obj.push(item);
            }
        })
    }
...
// 切换主题文件
export async function changeStyle (ele) {
    let theme = document.querySelector("#themeStyle")
    theme.href = `/static/themes/${localStorage.getItem('theme')}.css`
    let iframe = document.getElementsByClassName('jumpIframe');
    for (let item of iframe) {
        let theme = item.contentWindow.document.querySelector("#themeStyle")
        let stylee_6_1 = item.contentWindow.document.querySelector("#themestyle-6-1")
        theme.href = `/static/themes/${localStorage.getItem('theme')}.css`
        stylee_6_1.href = `/static/themes/${localStorage.getItem('theme')}-6-1.css`;
    }
}
