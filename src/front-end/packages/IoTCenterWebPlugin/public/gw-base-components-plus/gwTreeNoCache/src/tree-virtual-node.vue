<template>
  ...
  <div class="el-tree-node" @click.stop="handleClick" @contextmenu="($event) => this.handleContextMenu($event)"
    ...
    <div v-if="source.data.isGroup&&!source.data.isEquip&&(expanding||groupLoading)" class="block" @click.stop></div>
    <div class="el-tree-node__content">
      ...
      <div class="nodeContent" :style="
        ...
        <!-- 选框 -->
        <el-checkbox v-if="showCheckbox&&aleadyLoadAll" v-model="source.checked" :indeterminate="source.indeterminate"
        ...
        <span v-if="source.data.loading||(source.data.isGroup&&groupLoading&&source.level==1)" class="el-tree-node__loading-icon el-icon-loading"></span>
        ...
        <!-- 设备数量：仅分组展示 -->
        <span class="equipNumber" v-if="(source.data.isGroup &&!source.data.isEquip&& showCount)||(source.data.isGroup&&!source.data.isEquip&&searchName)">
          {{ source.data.count }}
        </span>
      </div>
    </div>
