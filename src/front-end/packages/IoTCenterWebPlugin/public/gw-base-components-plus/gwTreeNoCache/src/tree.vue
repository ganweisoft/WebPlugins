<template>
  <div class="gw-tree" v-loading="loading">
    ...
    <div class="el-tree" v-loading="searchLoading" :id="id" :key="id" :class="{
...
<script>
  ...
  import { getNodeKey, findNearestComponent, generateUUID } from './model/util'
  ...
  export default {
    ...
    watch: {
      ...
      data(newVal, oldVal) {
        if (window.top.storage) {
          for (let item in window.top.storage) {
            window.top.storage[item] = []
          }
        }
        ...
        // for (let item in this.nodeObject) {
        //   window.top.storage[this.nodeObject[item].title + '-' + this.nodeObject[item].key] = null
        // }
    ...
    methods: {
      ...
      initData() {
        ...
        for (let item in this.nodeObject) {
          if (this.nodeObject[item].isGroup) {
            ...
            if (window.top.storage && window.top.storage[sessionKey] && window.top.storage[sessionKey].length > 0) {
              try {
                let data = window.top.storage[sessionKey]
                // 将设备数据更新到树形结构
      ...
      updateEquipsWithCache(key) {
        ...
        let time = setTimeout(() => {
          // 若缓存中有该分组设备
          if (this.nodeObject[key] && window.top.storage[cacheName]) {

            //判断是否展开，展开则延迟加载 
            this.expandAndTimeoutLoad(key)
            // 扁平化设备信息，指向缓存
            this.flatternEquips(window.top.storage[cacheName])

            let equipNum = window.top.storage[cacheName].length
            ...
            this.aleadyLoadAll = this.allEquipNum <= this.statisticsNum
            // 设置分组所挂载的设备数量
            this.setGroupNum(key, window.top.storage[cacheName].length)
    ...
    mounted() {
      ...
      this.$on('node-expand', (item) => {
        if (!this.aleadyLoadAll && !window.top.storage[item.data.title + '-' + item.data.key]) {
        ...
        if (!item.expanded) {
          if (item.data.isEquip && this.needsSettings) {
            ...
            if (!item.data.groupUpdate && item.data.isGroup && window.top.storage[item.data.title + '-' + item.data.key]) {
