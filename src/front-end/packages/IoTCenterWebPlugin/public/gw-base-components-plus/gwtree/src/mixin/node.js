...
export default {
    ...
    methods: {
        creator (parent, nodeTag) {
            ...
            this.$on('node-expand', currentNode => {
                if (node !== currentNode) {
                    node.collapse();
                }
            });
        ...
        equipData(data){
            let returnData={}
            if(data.isEquip||(data.data&&data.data.isEquip)){
                returnData.title = data.title
                returnData.key = typeof data.key=='string'?Number(data.key.split('-')[1]):Number(data.key)
                returnData.staNo = data.staNo
                returnData.status = data.status
                returnData.isGroup = data.isGroup
                returnData.groupId = data.groupId
                returnData.groupName = data.parentTitle
                returnData.isEquip = data.isEquip
        ...
        handleExpandIconClick () {
            ...
            // this.tree.$emit('current-change', store.currentNode ? store.currentNode : null, store.currentNode,true);
        },
