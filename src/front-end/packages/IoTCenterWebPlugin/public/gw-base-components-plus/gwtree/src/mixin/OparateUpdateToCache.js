...
export default {
    methods: {
        ...
        deleteGroup (groupId, deleteChildEquips) {
            let arr = this.deepClone(window.top[this.nodeObject[groupId].title + '-' + groupId])
            if (!deleteChildEquips) {
                ...
                if (!window.top[cacheName]) {
                    window.top[cacheName] = []
                }
                for (let index = 0, length = arr.length; index < length; index++) {
                    window.top[cacheName].push(
                        {
                            key: arr[index].key,
            ...
            // 更新缓存
            if (window.top[this.nodeObject[groupId].title + '-' + groupId]) {
                window.top[this.nodeObject[groupId].title + '-' + groupId].length = 0
                delete window.top[this.nodeObject[groupId].title + '-' + groupId]
            }
        ...
        updateGroupNameFromCache (groupId, newName) {
            // 如果搜索状态下存在旧分组缓存，直接删除
            if (window.top[this.nodeObject[groupId].title + '-' + groupId + '-search']) {
                window.top[this.nodeObject[groupId].title + '-' + groupId + '-search'].length = 0
                delete window.top[this.nodeObject[groupId].title + '-' + groupId + '-search']
            }
            // 如果非搜索状态下，缓存中存在旧分组，则更新旧分组名称
            if (window.top[this.nodeObject[groupId].title + '-' + groupId]) {
                window.top[newName + '-' + groupId] = window.top[this.nodeObject[groupId].title + '-' + groupId]
                window.top[this.nodeObject[groupId].title + '-' + groupId] = null
                delete window.top[this.nodeObject[groupId].title + '-' + groupId]
            }
        ...
        addEquip (groupId, data) {
            if (this.nodeObject[groupId]) {
                ...
                data.forEach(item => {
                    ...
                    // 更新缓存
                    if (!window.top[this.nodeObject[groupId].title + '-' + groupId]) {
                        window.top[this.nodeObject[groupId].title + '-' + groupId] = []
                    }
                    window.top[this.nodeObject[groupId].title + '-' + groupId].unshift(
                        {
                            key: `${groupId}-${item.equipNo}`,
                            title: item.equipName,
                            staNo: item.staNo,
                ...
                this.flatternEquips(window.top[this.nodeObject[groupId].title + '-' + groupId])
                this.resetSelectNum()
        ...
        deleteEquips () {
            ...
            for (let item in groupObject) {
                groupObject[item].children.forEach(childItem => {
                    // 如果是搜索状态，则更新搜索状态下的缓存
                    if (this.searchName) {
                        let searchArr = window.top[this.nodeObject[item].title + '-' + item + '-search']
                    ...
                    // 更新非搜索状态下缓存
                    let arr = window.top[this.nodeObject[item].title + '-' + item]
                    for (let index = 0, length = arr.length; index < length; index++) {
        ...
        deleteCurrentEquip (groupName, groupNo, equipNo) {
            ...
            if (window.top[cacheGroupName]) {
                let arr = window.top[cacheGroupName];
        ...
        removeEqups (getKey) {
            ...
            for (let item in groupObject) {
                if (item != getKey) {
                    if (!window.top[this.nodeObject[getKey].title + '-' + getKey]) {
                        window.top[this.nodeObject[getKey].title + '-' + getKey] = []
                    }
                    if (this.searchName) {
                        if (!window.top[this.nodeObject[getKey].title + '-' + getKey + '-search']) {
                            window.top[this.nodeObject[getKey].title + '-' + getKey + '-search'] = []
                        }
                    }
                    // this.nodeObject[item].count = this.nodeObject[item].count - groupObject[item].children.length
                    groupObject[item].children.forEach(childItem => {
                        window.top[this.nodeObject[getKey].title + '-' + getKey].unshift({
                        ...
                        if (this.searchName) {
                            window.top[this.nodeObject[getKey].title + '-' + getKey + '-search'].unshift({
                                count: childItem.count,
                                groupId: getKey,
                                isGroup: childItem.isGroup,
                                key: childItem.key,
                        ...
                        if (this.searchName) {
                            let arr = window.top[this.nodeObject[item].title + '-' + item + '-search']
                            for (let i = 0, length = arr.length; i < length; i++) {
                                if (arr[i].key == childItem.key) {
                                    arr.splice(i, 1)
                                    break;
        ...
        recomputedGroupCount () {
            ...
            if (this.searchName) {
                for (let item in this.nodeObject) {
                    if (window.top[this.nodeObject[item].title + '-' + item + '-search']) {
                        let equipNum = window.top[this.nodeObject[item].title + '-' + item + '-search'].length
                ...
                for (let item in this.nodeObject) {
                    if (window.top[this.nodeObject[item].title + '-' + item]) {
                        let equipNum = window.top[this.nodeObject[item].title + '-' + item].length
                        this.setGroupNum(item, equipNum)
                    }
