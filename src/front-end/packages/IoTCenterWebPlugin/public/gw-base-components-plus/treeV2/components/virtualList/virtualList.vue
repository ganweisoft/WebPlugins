...
export default {
    ...
    methods: {
        ...
        init () {
            ...
            // 将树形结构扁平化到this.visibleList，传给虚拟滚动组件渲染
            this.updateTreeList()
            setTimeout(() => {
                ...
                // 过滤空分组（没有设备的分组）
                this.filterEmptyGroup()
            }, 100)
        },

        /**
         * @description 搜索条件下过滤空分组
         */
        filterEmptyGroup () {
            if (this.isSearchStatus) {
                let arr = [];
                Object.values(this.groupNodeObject).forEach(group => {
                    if (group.isGroup && !group.isEquip && !group.count) {
                        arr.push(group.key)
                    }
                })
                this.visibleList = this.visibleList.filter(item => !arr.includes(item.key))
            }
        },
        ...
        filterWithAlias (groupKey) {
            if (this.aliasName) {
                let arr = window.top[`group-${groupKey}`] || []
                window.top[`group-${groupKey}${this.aliasName}`] = this.filterData(arr)
            }
        },
        ...
        updateCurrentSelect () {
            if (window.top.equipCache && window.top.equipCache[this.currentNodeKey]) {
                this.currentSelect = `${window.top.equipCache[this.currentNodeKey].groupId}-${this.currentNodeKey}`
            }
        },
        ...
        updateGroupEquips (key, isInit) {
            ...
            if (isInit) {
                let arr = window.top[`group-${key}${this.aliasName}`] || []
            ...
            if (this.showStatus && this.equipStatusManage) {
                this.equipStatusManage.updateGroupStatusByKey(key)
            }
        },
        ...
        /**
         *
         * @param {Object} node 节点el-checkbox选中
         * @description 节点选中事件
         */
        onChecked (node) {
        ...
        /**
         *
         * @param {Boolean} isGroupNew 是不是新增分组，是:true,否:false,false的话就是编辑分组名称
         * @param {Object} node 分组节点
         * @description 编辑分组
         */
        ...
        /**
         * @description 通过传过来的设备控制项，更新el-checkbox选中状态
         */
        ...
        /**
         * @description 通过传过来的设备选中列表，更新el-checkbox选中状态
         */
        ...
        filterMethod (searchName) {
            ...
            this.equipNumManage.resetGroupNum(this.isSearchStatus)
            this.equipStatusManage && this.equipStatusManage.countGroupStatus(this.isSearchStatus)
        ...
        */
        DeleteEquip (data) {
            ...
            if (this.equipStatusManage) {
                this.equipStatusManage.reRenderGroupStatus()
            }
        },
