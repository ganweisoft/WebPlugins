...
export default {
    ...
    methods: {
        init () {
            ...
            // 传入数据生成 option
            this.option = this.getPie3D(this.data, 0.8);
            ...
            this.myChart.on('legendselectchanged', (params) => {
                ...
                this.option = this.getPie3D(copyData, 0.8);
                this.option = this.merge(this.option, this.layoutOptions)
        ...
        getParametricEquation (startRatio, endRatio, isSelected, isHovered, k, h) {
            ...
            return {
                ...
                x: function (u, v) {
                    if (u < startRadian) {
                        return offsetX + Math.cos(startRadian) * (2 + Math.cos(v) * k) * hoverRate;
                    }
                    if (u > endRadian) {
                        return offsetX + Math.cos(endRadian) * (2 + Math.cos(v) * k) * hoverRate;
                    }
                    return offsetX + Math.cos(u) * (2 + Math.cos(v) * k) * hoverRate;
                },
                y: function (u, v) {
                    if (u < startRadian) {
                        return offsetY + Math.sin(startRadian) * (2 + Math.cos(v) * k) * hoverRate;
                    }
                    if (u > endRadian) {
                        return offsetY + Math.sin(endRadian) * (2 + Math.cos(v) * k) * hoverRate;
                    }
                    return offsetY + Math.sin(u) * (2 + Math.cos(v) * k) * hoverRate;
                },
                z: function (u, v) {
                    if (u < -Math.PI * 0.5) {
                        return Math.sin(u);
