...
export default class checkStatusManage extends WindowManager {
    ...
    reComputedCheckNum (isSearchStatus) {
        ...
        if (this.isSearchStatus) {
            ...
            // 3:依照搜索结果,重新计算分组选中的设备数据量
            Object.values(this.groupNodeObject).forEach(item => {
                let list = this.window[`group-${item.key}-search`] || []
                list.forEach(equip => {
    ...
    updateCheckedStatusWithEquips (list) {
        window.setTimeout(() => {
            for (let index = 0, length = list.length; index < length; index++) {
                // 从缓存中看有没有该设备，防止是脏数据
                let data = this.window.equipCache && this.window.equipCache[list[index]]
    ...
    updateCheckedStatusWithControls (list) {
        window.setTimeout(() => {
            ...
            for (let index = 0, length = list.length; index < length; index++) {
                ...
                if (this.window.equipCache && this.window.equipCache[equipNo]) {
                    let groupId = this.window.equipCache[equipNo].groupId
    ...
    updateGroupSelect (list, checked) {
        list.forEach(item => {
            if (this.groupNodeObject[item]) {
                ...
                let list = this.window[`group-${item}${this.aliasName}`] || []
                if (this.isSearchStatus) {
                    list = this.window[`group-${item}-search`] || []
                }
